---
title: "Careers / Teaching Fellow Positions Open at DAU / Application Form Submission Link"
url: "https://forms.gle/uz2z4RWWtkRJW8ZE6"
category: "Academics"
scraped_by: "Meet Virugama"
scraped_date: "2026-05-31"
team: "Team D"
---

# Overview

This document contains the schema and details of the online application form for Teaching Fellow positions for Academic Year 2026-27 at Dhirubhai Ambani University.

# Main Content

### Teaching Fellow Application AY2026-27

This application is hosted on Google Forms and is used by the university administration to collect profiles of applicants for Teaching Fellow positions.

**Application Deadline:** June 30, 2026

#### Form Questionnaire Schema

| Field Section / Label | Type | Requirement / Options |
| --- | --- | --- |
| **Email** | Short Text | Required (Account email verification) |
| **Name** | Short Text | Required |
| **E-mail** | Short Text | Required |
| **Address** | Paragraph Text | Required |
| **Cell Phone Number** | Short Text | Required |
| **Date of Birth** | Date (dd/mm/yyyy) | Required |
| **Undergraduate (UG) Degree (BTech, BE, etc.) and Institute Name** | Short Text | Required |
| **Post-Graduate (PG) Degree (ME, MTech, etc.) and Institute Name** | Short Text | Required |
| **PhD Degree and Institute Name** | Short Text | Optional |
| **Current Employer** | Short Text | Optional |
| **Year of Experiences** | Short Text | Optional |
| **Area of Interest** | Radio Choice | Required. Options:<br>• Computer Science & Information Technology<br>• Data Science & Artificial Intelligence<br>• Electronics & Communication Engineering<br>• Electronics and VLSI<br>• Humanities and Social Sciences (HSS) |
| **Attach your CV** | File Upload | Required (Max 10 MB) |

# Related Links

- [Google Form - Teaching Fellow Application AY2026-27](https://forms.gle/uz2z4RWWtkRJW8ZE6)
- [DAU Teaching Fellow Careers Page](https://www.daiict.ac.in/teaching-fellow-positions-open-dau)

