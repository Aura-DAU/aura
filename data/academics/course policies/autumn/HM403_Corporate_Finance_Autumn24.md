---
title: "HM403 Corporate Finance Autumn 2024"
url: "https://ecampus.daiict.ac.in/webapp/intranet/index.jsp"
category: "Academics - Course Policies"
scraped_by: "Squad D Scraper"
scraped_date: "2025-07-10"
team: "Squad D"
source_type: "PDF"
pdf_name: "HM403_Corporate Finance_Autumn24 - Cyril Jos Nazhicheril.pdf"
course_code: "HM403"
semester: "Autumn 2024"
authorization: ["public"]
---

# HM403: Corporate Finance

## Course Overview

| Field | Details |
|---|---|
| **Course Code** | HM403 |
| **Course Name** | Corporate Finance |
| **Instructor(s)** | Cyril Jos Nazhicheril |
| **Credits** | 3-0-0-3 (Lecture hours/week: 3, Tutorial hours/week: 0, Practical hours/week: 0, Total Credits: 3) |
| **Semester Offered** | Autumn 2024 |
| **Type** | HASS Open Elective |
| **Program(s)** | B.Tech 7th Semester; M.Sc. 3rd Semester |
| **Year / Semester in Program** | B.Tech — 7th Semester; M.Sc. — 3rd Semester |
| **Associated Lab** | None |
| **Prerequisites** | None |
| **Foundation For** | Courses in Advanced Quantitative Finance |

---

## Course Description

The course Corporate Finance introduces the corporation and emphasizes value creation as its goal while describing the agency issues that may arise. It discusses the mechanics of discounting cash flows and the concept of present value, which has applications to personal finance, valuation of bonds and stocks, as well as projects undertaken by the firm. The course also establishes the intuition for understanding risk and return and introduces the Capital Asset Pricing Model. The section on financial statements introduces key financial statements, and the coverage of financial ratios helps students analyze them holistically. The course thus lays the foundation for students aspiring for MBA and finance-related Master's degrees.

---

## Course Outcomes (COs)

*Note: No explicit numbered Course Outcomes (COs) list is present in the source document. The course's intended learning value is described in the Abstract Content (Course Description above): understanding value creation, agency issues, discounting cash flows, present value, bond/stock valuation, risk and return, the Capital Asset Pricing Model, and financial statement analysis.*

---

## Program Outcome Mapping (PO Mapping)

*Note: No PO mapping table is present in the source document.*

---

## Course Structure

| Unit | Topic | No. of Lectures |
|---|---|---|
| Introduction to Corporate Finance | Principles of corporate finance, types of firms, goal of financial management, agency relationships, corporate governance | 5 |
| Financial Decision Making | Time value of money, law of one price, net present value, internal rate of return, perpetuities and annuities, economic cost and accounting cost | 5 |
| Interest Rates and Bond Valuation | Nominal and real interest rate, types of bonds, yield curve, dynamic behaviour of bond prices, interest rate risk, bond ratings | 5 |
| Stock Valuation | Features of common and preferred stock, valuation models (dividend discount model, total payout model, valuation using multiples) | 4 |
| Valuing Projects | Net present value rule, IRR rule, payback rule, project selection with resource constraints | 3 |
| Sources of Finance | Internal sources of finance (retained profit, sale of assets, etc.), external sources of finance (share capital, venture capital, debt) | 4 |
| Risk and Return | Capital markets and the pricing of risk, optimal portfolio choice and the capital asset pricing model, estimating cost of capital | 5 |
| Financial Statement Analysis | Balance sheet, income statement, statement of cash flows, ratio analysis | 5 |
| Special Topics | International Corporate Finance, behavioural finance, enterprise risk management, leasing | 5 |
| **Total** | | **41** |

---

## Weekly Schedule / Lecture Plan

*Note: No weekly schedule with dates is available in the source document. Only a topic-wise lecture plan (Detailed Course Contents) is provided above.*

---

## Evaluation / Grading Scheme

| Component | Weightage |
|---|---|
| In-Semester Examination | 30 marks |
| End-Semester Examination | 50 marks |
| Assignments/Presentations/Quizzes | 20 marks |
| **Total** | **100 marks** |

---

## Textbooks and References

### Textbook(s)

1. Jonathan Berk and Peter DeMarzo, *Corporate Finance*.
2. Stephen Ross and Randolph Westerfield, *Fundamentals of Corporate Finance*.

### Reference Books

*None stated separately in source document.*

### Online Resources

*None stated in source document.*

---

## Program Structure Context

HM403 Corporate Finance is a HASS Open Elective offered to B.Tech students in their 7th semester and M.Sc. students in their 3rd semester. It has a 3-0-0-3 credit structure with no prerequisites, and serves as a foundation for courses in Advanced Quantitative Finance.

---

## Additional Notes

- The source document's third page is blank (contains no additional content).
- The source document is titled "Template for Course Offering," indicating it may follow a standardized DA-IICT course proposal format.
- No explicit Course Outcomes (CO) list or PO mapping table is provided in the source document.

---

## Downloads and Resources

| Resource | Type | Link |
|---|---|---|
| HM403_Corporate Finance_Autumn24 - Cyril Jos Nazhicheril.pdf | PDF | [Download HM403_Corporate Finance_Autumn24 - Cyril Jos Nazhicheril.pdf](https://ecampus.daiict.ac.in/webapp/intranet/index.jsp) |

---

## Document Metadata

| Field | Value |
|---|---|
| **Source PDF** | HM403_Corporate Finance_Autumn24 - Cyril Jos Nazhicheril.pdf |
| **Scraped Date** | 2025-07-10 |
| **Intranet Portal** | [DA-IICT Intranet](https://ecampus.daiict.ac.in/webapp/intranet/index.jsp) |
| **Academic Guidelines** | [DAU Academics](https://daiict.ac.in/academics) |
