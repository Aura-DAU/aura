# Timetable — B.Tech ICT-CS — 1st Year (Autumn 2026-27)

Source: derived from the official DAU Autumn 2026-27 combined lecture timetable, filtered by course code.

## Important context

DA-IICT's 1st-year curriculum runs on a shared "Common Curriculum Framework" — Intro to ICT, Calculus, Programming, Basic Electronic Circuits, Language & Literature are the same Institute Core courses for every branch, split into four room-capacity sections (A–D) that mix students across branches. The only branch-specific signal in the raw timetable data is a set of six parallel "PC1" (Program Core 1) sessions, all run at the same three time slots (Mon 9, Wed 8, Thu 10), each in a different room:

- PC1 (ICT) — Rm CEP-106, faculty RM
- PC1 (MNC) — Rm CEP-204, faculty VS
- PC1 (EVD) — Rm CEP-205, faculty AR
- PC1 (CS-AI) — Rm CEP-103, faculty SCN
- PC1 (unlabeled #1) — Rm CEP-108, faculty SB2
- PC1 (unlabeled #2) — Rm CEP-206, faculty AB

**Flag — please verify:** the source timetable has no PC1 row explicitly labeled "ICT-CS." I've assigned the first unlabeled PC1 session (faculty SB2, Rm CEP-108) to ICT-CS as a best guess, since it's one of two unlabeled sessions alongside "ECE-AI" below and DA-IICT does list ICT-CS as a direct-entry option in some years. If SB2 doesn't ring a bell as an ICT/CS faculty member to you, this is very possibly swapped with the ECE-AI file — check faculty initials against your registrar's list and swap if needed.

## Program Core (PC1) — ICT-CS
...
These four courses are mandatory for every 1st-year B.Tech student regardless of branch, split into four room-capacity sections (A–D) that mix students from all branches. Included here for completeness.
