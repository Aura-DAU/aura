---
title: "Workshop on RTL to GDS-II: VLSI Design and Hardware Security for Trusted Memory Systems"
url: "https://www.daiict.ac.in/events/7059"
category: "Events"
scraped_by: "Dhruvam"
scraped_date: "2026-06-01"
team: "Team 2"
---

# Workshop on RTL to GDS-II: VLSI Design and Hardware Security for Trusted Memory Systems

# Overview

Dhirubhai Ambani University (DAU) , Gandhinagar, Gujarat, India is hosting a one-week workshop titled “ RTL to GDS-II: VLSI Design and Hardware Security for Trusted Memory Systems ” from 06 th to 10 th July 2026 .

# Main Content

# Workshop on RTL to GDS-II: VLSI Design and Hardware Security for Trusted Memory Systems

Dhirubhai Ambani University (DAU) , Gandhinagar, Gujarat, India is hosting a one-week workshop titled “ RTL to GDS-II: VLSI Design and Hardware Security for Trusted Memory Systems ” from 06 th to 10 th July 2026 .

This program is designed to offer participants a comprehensive understanding of the RTL to GDS-II design flow , along with key concepts in VLSI design, hardware security and memory systems . The course combines theoretical insights with extensive hands-on training sessions.

The workshop is tailored for industry professionals, faculty members, Ph.D. scholars, postgraduate students , and senior undergraduate students. Participants will gain practical exposure through hands-on sessions using Cadence and SPICE EDA tools .

Register Now

Participants from Gujarat are eligible for a full refund of the ₹5,000 registration fee , subject to maintaining at least 75% attendance during the workshop.

# For any queries, please contact :

# Workshop Coordinators:

Prof. Sreeja Rajendran (+91-9074703191 | sreeja_rajendran@dau.ac.in )

Prof. Yash Agrawal (+91-9882114669 | yash_agrawal@dau.ac.in )

# AIP Cell, DAU:

Mr. Pankit Gandhi (+91-079-68261666 | aip@dau.ac.in )

# Images

[Image Present: Workshop on RTL to GDS-II: VLSI Design and Hardware Security for Trusted Memory Systems]
[Image Present: Summer School on Automatic Speech Recognition during July 04-08, 2020]

# Important Information

_No important information found._

# Contact Information

- **Email:** aip@dau.ac.in
- **Email:** sreeja_rajendran@dau.ac.in
- **Email:** yash_agrawal@dau.ac.in
- **Phone:** +91-079-68261666
- **Phone:** +91-9074703191
- **Phone:** +91-9882114669
