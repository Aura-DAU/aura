...
import { askAura, ChatMessage, StudentProfile, Citation } from "@/lib/api/chat.action";
...
export function useAuraChat(options: UseAuraChatOptions = {}) {
  const { storageKey = "aura_chat_history", profileKey = "aura_student_profile" } = options;
  ...
  // TODO (Issue #104): replace hardcoded mock student Rahul Sharma with authenticated session profile
  ...
  const handleSendMessage = async (textToSend: string) => {
    ...
    if (!recentThreads.includes(threadTitle)) {
      ...
      localStorage.setItem("aura_recent_threads", JSON.stringify(updatedThreads));
    }
    ...
    try {
      ...
      if (result.success && result.content) {
        ...
        setErrorMessage("Failed to receive response from AURA server.");
      }
    } catch {
      saveHistory([
        ...updatedMessages,
