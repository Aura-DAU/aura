...
from chunk_quality import is_low_quality_chunk
...
def process_markdown_file(file_path):
    ...
    # Fix #2 (RAG_Final_Benchmark_Report): "Where is our alumnus [Name]
    # working now?" queries were failing with "could not find that
    # information" even though a dedicated per-person profile file exists
    # for every one of the names checked (data/alumni/alumni_<name>.md).
    # The content was never the problem — there was simply no entity-index
    # field to look a specific alumnus up by name, unlike faculty_name.
    # "Alumni Profile" (per-person) is distinct from "Alumni Batch Index"
    # (a roster/table of many alumni) — only the former gets a clean single
    # alumni_name; a batch-listing file legitimately mentions dozens of
    # names and isn't "about" any single one of them.
    alumni_name = None
    if category == "alumni profile":
        alumni_name = metadata.get("title") or file_path.stem.replace("alumni_", "").replace("_", " ").title()
    ...
    for section in sections:
        ...
        for chunk_text in split_chunks:
            # Fix #9: skip chunks with too little substantive content to be
            # worth storing/embedding (e.g. a lone table caption, a heading
            # with no body, a whitespace-only split artifact).
