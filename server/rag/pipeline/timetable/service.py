...
def _require_cohort(identity) -> tuple[int, int, str]:
    ...
    # Resilient fallback: infer from erp_id pattern (e.g. 202401226 -> Year 3, Sem 5, Sec A)
    if (year is None or sem is None or not sec) and erp_id:
        import re, datetime
        match = re.match(r"^(\d{4})", str(erp_id))
        if match:
            ...
            calc_year = max(1, min(4, academic_year - admission_year + 1))
            calc_sem = (calc_year - 1) * 2 + (1 if month >= 7 else 2)
            year = year if year is not None else calc_year
            sem = sem if sem is not None else calc_sem
            sec = sec if sec else "A"

    # Final safe default for demo testing
    year = year if year is not None else 2
    sem = sem if sem is not None else 3
    sec = sec if sec else "A"
